...
$is_valid = validate($playerData, $errors, $input);

function validate(&$playerData, &$errors, $input) {
    if (empty($input['name']) || strlen(trim($input['name'])) < 4 || !preg_match('/^[a-zA-Z ]+$/', trim($input['name']))) {
        $errors[] = 'Provide a name with at least 4 characters and no numbers or special characters!';
    }

    if (!isset($input['goals2024']) || !filter_var($input['goals2024'], FILTER_VALIDATE_INT)) {
        $errors[] = 'Provide a valid goal number for 2024!';
    }

    if (empty($input['positions'])) {
        $errors[] = 'Provide at least one position!';
    }

    if (count($errors) === 0) {
        $playerData['name'] = trim($input['name']);
        $playerData['goals2024'] = $input['goals2024'];
        $playerData['positions'] = $input['positions'];
    }
